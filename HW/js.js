...
function getPage(pageOption, searchOption, seriesOption, movieOption, episodeOption) {
    ...
    fetch(url)
        ...
        .then(function (data) {
            Rdata = data;
            $('#content').html(appendData(Rdata));
            appendPages(Rdata, pageOption);
            pagesLoad()
            appendFavButtonList()
            appendFavIcon(Rdata);
            addInfoClick();
...
function appendPages(data, pageOption) {
    // pages loop
...
function appendFavIcon(data) {
    if (data['Response'] == 'False') $('#content').append(`<h1 class="favorite"> Nothing found.. Try something else </h1>`)
    else {
        for (let i = 0; i < data['Search'].length; i++) {
            // Favorite star click and push to local storage
